...
std::string token_to_piece(llama_token token) {
    ...
    const int n = llama_token_to_piece(g_state.vocab, token, result.data(), static_cast<int>(result.size()), 0, false);
    if (n <= 0) {
        return std::string();
    }
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    log_info("✅ libllama_jni.so JNI_OnLoad called - Library loaded successfully");
    return JNI_VERSION_1_6;
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_recemotion_LLMInferenceHelper_nativeInit(JNIEnv * env, jobject /*thiz*/, jstring modelPath, jint nCtx, jint nThreads) {
    ...
    if (!g_state.model) {
        log_error("Failed to load model - file may be corrupted or invalid GGUF format");
    ...
    if (!g_state.ctx) {
        log_error("Failed to create context - insufficient memory or invalid parameters");
        release_state();
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_recemotion_LLMInferenceHelper_nativeGenerate(JNIEnv * env, jobject /*thiz*/, jstring promptText, jint maxTokens) {
    std::lock_guard<std::mutex> lock(g_mutex);
    ...
    for (int i = 0; i < max_tokens && n_past < g_state.n_ctx - 4 && !stop_generation; ++i) {
...
Java_com_example_recemotion_LLMInferenceHelper_nativeRelease(JNIEnv * /*env*/, jobject /*thiz*/) {
    std::lock_guard<std::mutex> lock(g_mutex);
    release_state();
    if (g_backend_initialized) {
        llama_backend_free();
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_recemotion_LLMInferenceHelper_nativeInitTFLite(JNIEnv * env, jobject /*thiz*/, jstring modelPath) {
    // TODO: Implement TensorFlow Lite model initialization
    // For now, return error message
    log_error("TensorFlow Lite support not yet implemented");
    return env->NewStringUTF("TensorFlow Lite support not yet implemented. Please use .gguf format.");
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_example_recemotion_LLMInferenceHelper_nativeGenerateTFLite(JNIEnv * env, jobject /*thiz*/, jstring promptText, jint maxTokens) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_recemotion_LLMInferenceHelper_nativeReleaseTFLite(JNIEnv * /*env*/, jobject /*thiz*/) {
